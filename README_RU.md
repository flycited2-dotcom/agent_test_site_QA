...
Кнопка `Режим` меняет именно плановый режим: если выбран `critical`, планировщик больше не запускает smoke каждые 30 минут; если выбран `full`, запускается полный режим по `FULL_INTERVAL_MINUTES` и дневному часу `FULL_RUN_HOUR`. Кнопка `Запустить сейчас` запускает разовую проверку независимо от выбранного расписания. Одновременно выполняется только один прогон: если проверка ещё идёт, новый запуск пропускается.
...
В `critical` и `full` режимах агент дополнительно проверяет сценарии намерений:

```text
shop/catalog/auto: главная → каталог/поиск → категория/карточка → целевое действие
landing/content/auto: главная → CTA/якоря → форма или контакты
```

Для поиска используются слова из `QA_SEARCH_TERMS`, например:

```text
QA_SEARCH_TERMS=кондиционер,товар,услуга
```

Если сценарий падает, в отчёт добавляется `journey-steps.txt` со списком шагов воспроизведения.

В `full` режиме дополнительно запускается глубокий commerce-аудит:

```text
каталог → вложенные категории → фильтры → поиск по словам/модели → сортировки → карточки → количество → заказ/корзина → контакты
```

Глубина регулируется переменными:

```text
MAX_DEEP_PRODUCT_URLS=80
MAX_DEEP_CATALOG_URLS=40
DEEP_AUDIT_BUDGET_MS=1800000
```

Агент не совершает реальные звонки/SMS и не оплачивает заказы. Он проверяет ссылки `tel:`, `sms:`, `mailto`, мессенджеры, формы и безопасные действия до финального опасного шага.
